...
public class DexprDivision extends AbstractMathOperator {

    public DexprDivision(Dexpr left, Dexpr right) {
        super(DexprType.OPERATOR_DIV, left, right);
    }

    protected Dexpr evaluate(double left, double right) throws DexprException {
        if (right == 0.)
            throw new DexprException("division (/) by zero (0)");
